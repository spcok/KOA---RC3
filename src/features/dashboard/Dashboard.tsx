    ...
    <div className="p-4 md:p-8 space-y-6 w-full max-w-7xl mx-auto font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 flex items-center gap-2">
            {new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} <span className="text-slate-300">|</span> 🌤️ 14°C Partly Cloudy
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col transition-all duration-300">
              <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsBentoMinimized(!isBentoMinimized)}>
                  <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-sm font-semibold text-slate-800">Pending Duties</h2>
                  </div>
                  <div className="flex items-center gap-3">
                      <span className="bg-slate-100 text-slate-600 text-xs font-bold px-2.5 py-1 rounded-md">{taskStats?.pendingTasks?.length || 0}</span>
                          ...
                          (taskStats?.pendingTasks || []).map(t => (
                              <div key={t.id} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-100 hover:border-slate-200 transition-colors">
                                  ...
                                  <div>
                                      ...
                                      <p className="text-xs text-slate-500 mt-1">Due: {getSafeDate(t.due_date)}</p>
                                  </div>
                              </div>
                          ))
                      ) : (
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col transition-all duration-300">
              <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsBentoMinimized(!isBentoMinimized)}>
                  <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-sm font-semibold text-slate-800">Health Rota</h2>
                  </div>
                  <div className="flex items-center gap-3">
                      <span className="bg-rose-50 text-rose-600 text-xs font-bold px-2.5 py-1 rounded-md">{taskStats?.pendingHealth?.length || 0}</span>
                      <button className="text-slate-400 hover:text-slate-600">
                          {isBentoMinimized ? <ChevronDown size={20} /> : <ChevronUp size={20} />}
                      </button>
                  </div>
                      ...
                      {(taskStats?.pendingHealth?.length || 0) > 0 ? (
                          (taskStats?.pendingHealth || []).map(t => (
                              <div key={t.id} className="flex items-start gap-3 p-3 rounded-lg bg-rose-50/50 border border-rose-100 hover:border-rose-200 transition-colors">
                                  ...
                                  <div>
                                      <p className="text-sm font-medium text-slate-800 leading-tight">{t.title}</p>
                                      <p className="text-xs text-slate-500 mt-1">Mandatory: {getSafeDate(t.due_date)}</p>
                                  </div>
                              </div>
                          ))
                      ) : (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-[#0fa968] rounded-xl p-6 text-white flex justify-between items-center shadow-sm">
          <div>
            <div className="text-sm font-medium opacity-90 mb-1">Weighed Today</div>
            <div className="text-4xl font-bold">
              {animalStats?.weighed || 0}<span className="text-xl opacity-80">/{animalStats?.total || 0}</span>
            </div>
          </div>
        ...
        <div className="bg-[#f97316] rounded-xl p-6 text-white flex justify-between items-center shadow-sm">
          <div>
            <div className="text-sm font-medium opacity-90 mb-1">Fed Today</div>
            <div className="text-4xl font-bold">
              {animalStats?.fed || 0}<span className="text-xl opacity-80">/{animalStats?.total || 0}</span>
            </div>
          </div>
          <div className="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm gap-4">
        <div className="flex items-center gap-4 overflow-x-auto w-full md:w-auto">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { const d = new Date(viewDate); d.setDate(d.getDate() - 1); setViewDate(d.toISOString().split('T')[0]); }} className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm hover:bg-slate-50 whitespace-nowrap">← Previous</button>
            <div className="relative">
              <input type="date" value={viewDate} onChange={e => setViewDate(e.target.value)} className="pl-3 pr-10 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <Calendar size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
            </div>
            <button onClick={() => { const d = new Date(viewDate); d.setDate(d.getDate() + 1); setViewDate(d.toISOString().split('T')[0]); }} className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm hover:bg-slate-50 whitespace-nowrap">Next →</button>
            <button onClick={() => setViewDate(new Date().toISOString().split('T')[0])} className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm hover:bg-slate-50 whitespace-nowrap">Today</button>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={cycleSort} className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-sm font-medium hover:bg-slate-50 text-slate-700 bg-white min-w-[100px]">
        ...
        {edit_animals && (
          <button onClick={() => setIsCreateAnimalModalOpen(true)} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 shadow-sm whitespace-nowrap w-full md:w-auto">
      ...
      <div className="flex items-center justify-between pt-4">
        <h2 className="text-2xl font-semibold text-slate-800">Your {activeTab.charAt(0) + activeTab.slice(1).toLowerCase()}</h2>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-white border-b border-slate-200 text-slate-600 font-medium">
              <tr>
                ...
                <th className="px-6 py-4 whitespace-nowrap">Species</th>
                <th className="px-6 py-4 whitespace-nowrap">Ring #</th>
                <th className="px-6 py-4 whitespace-nowrap">Status</th>
                <th className="px-6 py-4 whitespace-nowrap">Today</th>
                <th className="px-6 py-4 whitespace-nowrap">Latest</th>
                <th className="px-6 py-4 whitespace-nowrap">Target</th>
                <th className="px-6 py-4 whitespace-nowrap">Fed</th>
              ...
              {(filteredAnimals || []).map(animal => {
                ...
                return (
                  <tr key={animal.id} className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => onSelectAnimal(animal)}>
                    <td className="px-6 py-4 font-semibold text-slate-900 whitespace-nowrap">{animal.name}</td>
                    <td className="px-6 py-4 text-slate-500 whitespace-nowrap">{animal.species}</td>
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{animal.ring_number || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-medium">active</span>
                    </td>
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{d?.todayWeight ? getWeightDisplay(d.todayWeight, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{d?.previousWeight ? getWeightDisplay(d.previousWeight, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{animal.flying_weight_g ? formatWeightDisplay(animal.flying_weight_g, animal.weight_unit) : '-'}</td>
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap">{d?.todayFeed ? (typeof d.todayFeed.value === 'string' ? d.todayFeed.value : String(d.todayFeed.value || 'Fed')) : '-'}</td>
                    <td className="px-6 py-4 text-blue-500 whitespace-nowrap">{animal.location}</td>
